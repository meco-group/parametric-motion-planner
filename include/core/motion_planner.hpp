...
class MotionPlanner{
    public:
        ...
        Point2D<double> GetStartVel() const { return start_vel_;};
        double GetVehWidth() const { return params_.GetVehWidth();};
        double GetVehHeight() const { return params_.GetVehHeight();};
        std::shared_ptr<const Trajectory> GetLastSolution() const { return emergency_mode_ ? emergency_solution_ : last_solution_;};
        ...
        void ResetEmergencyMode(){emergency_mode_ = false;};
        void SetCachedTrajectory(std::shared_ptr<Trajectory> traj){
            last_solution_ = traj;
            sample_ptr_ = 0;
        };
    ...
    private:
        ...
        std::shared_ptr<Trajectory> last_solution_;
        int sample_ptr_ = 0;
        std::shared_ptr<Trajectory> emergency_solution_;
        int emergency_sample_ptr_ = 0;
        bool emergency_mode_ = false;
        std::shared_ptr<Trajectory> previous_solution_;
        int previous_sample_ptr_ = 0;

        std::shared_ptr<Trajectory> reset_trajectory_;
