...
inline std::string TaskEventTypeToString(TaskEventType type) {
    switch (type) {
        ...
        case TASK_ABORTED: return "TASK_ABORTED";
        case TASK_RESUMED: return "TASK_RESUMED";
...
class MoverTask{
    public:
        ...
        bool IsDeadlockResolutionTask() const { return deadlock_resolution_task_; }
        bool HasBeenCompleted() const { return completed_; }
        ...
        void NotifyResumed(double current_time);
        ...
        friend std::ostream& operator<<(std::ostream& os, const MoverTask& task) {
            os << "MoverTask(agent_idx: " << task.agent_idx_ 
               << ", sequence_nb: " << task.task_sequence_nb_
               << ", destination: " << task.destination_name_ 
               << ", time_to_reveal: " << task.time_to_reveal_task_ 
               << ", delay: " << task.task_delay_ 
               << ", revealed: " << task.has_been_revealed_
