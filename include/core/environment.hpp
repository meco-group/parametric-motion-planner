...
class Environment{
    public:
        ...
        bool HasDeletedNeighbor(Point2D<int> cell) const;
        ...
        std::string GetNearestFreeClaimableDestination(const Point2D<double> &pos,
            bool ignore_neighbouring_destinations=true) const;
        void ClearClaimableDestinations(){claimable_destinations_.clear();};
        int GetNbDestinationsClaimedBy(const void* caller) const;
        void PrintDestinationsClaimedBy(const void* caller) const;
        ...
        // Update the obstacles in the local view of the environment to 
        // represent a current view of the environment, seen by the caller
        // (claimed destinations are free, all others are obstacles)
        void UpdateLocalView(Environment& env_view, const void* caller);

        Environment Clone() const {
            Environment env_copy(nb_cell_rows_, nb_cell_cols_, 
                                 cell_width_, cell_height_);
            env_copy.occupancy_grid_ = occupancy_grid_;
            return env_copy;
        };
