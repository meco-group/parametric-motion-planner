...
void Trajectory::GetBoundingBox(int start_idx, int stop_idx, Corridor& box) const {
    start_idx = std::max(0, std::min(start_idx, curr_nb_samples_ - 1));
    stop_idx = std::max(0, std::min(stop_idx, curr_nb_samples_ - 1));

    const auto [x_min, x_max] = std::minmax_element(px_.begin() + start_idx, px_.begin() + stop_idx + 1);
    const auto [y_min, y_max] = std::minmax_element(py_.begin() + start_idx, py_.begin() + stop_idx + 1);
