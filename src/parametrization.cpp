...
void Parametrization::PrepareOptiInstances(const UpdateToken&, 
	...
	for (int i = 1; i < 9; i++){
		...
		if (prepare_sparsely_){
			movable_points_code = '0' + std::string(i-1, '0') + '0';
			PrepareSingleOptiInstance(i, solver_name_, opts_casadi, opts_solver, movable_points_code);
			movable_points_code = '0' + std::string(i-1, '1') + '0';
			PrepareSingleOptiInstance(i, solver_name_, opts_casadi, opts_solver, movable_points_code);
		} else {
...
void Parametrization::Solve(const UpdateToken&, std::string& solver_name, 
	...
	if (use_warm_start){
		...
		if (just_in_time_preparation_mode || 
			...
			if (just_in_time_preparation_mode){
				...
				code = "0"; for (int i = 0; i < n-1; i++){code += "1";} code += "0";
			}
	...
	bool SOLVE_IN_THREAD = false;
	std::vector<DM> output;
	if (SOLVE_IN_THREAD){
		bool done = false;
		std::thread worker([&](){
			output = active_opti_instance_(input);
			done = true;
		});
		auto start = std::chrono::high_resolution_clock::now();
		while (!done){
			if (std::chrono::duration_cast<std::chrono::seconds>(
					std::chrono::high_resolution_clock::now() - start).count() > 0.1) {
				std::cout << "WARNING: optimization taking too long, aborting..." << std::endl;
				worker.detach();
				throw std::runtime_error("Optimization timeout");
			}
...
void Parametrization::PrepareSingleOptiInstance(int nbCorridors,
	...
	// 	opti_f = Function::load(name + ".casadi");
	...
	// 	return;
	// } catch (...){
	// 	// std::cout << "\t\tpreparing new opti instance" << std::endl;
	// }
