...
MotionPlanner::MotionPlanner(PlannerMethod method, Parameters const &params, 
    ...
    last_solution_ = std::make_shared<Trajectory>();
    emergency_solution_ = std::make_shared<Trajectory>();
    reset_trajectory_ = std::make_shared<Trajectory>();
}
...
void MotionPlanner::UpdateCorridorSequence(){
    if (locked_corridor_sequence_){
        ...
        corridor_sequence_.UpdateCorridorIdxs(start_, dest_, start_vel_);
        return;
    }
...
void MotionPlanner::Plan(){
    ...
    // clear last_solution (otherwise while writing last_solution, we overwrite previous_solution as well)
    last_solution_ = std::make_shared<Trajectory>();
    ...
    auto true_planning_computation_time_start = std::chrono::high_resolution_clock::now();
    switch(method_){
        case P2P:
            PlanP2P();
            break;
    ...
    std::chrono::duration<double, std::milli> true_planning_computation_time =
        planning_computation_time_end - true_planning_computation_time_start;
    ...
    // std::cout << "True planning computation time (excluding corridor update): " << true_planning_computation_time.count() << " ms" << std::endl;
    }
    ...
    if (USE_THREADED_PLANNING){
        ...
        try{
            ...
            if (start_vel_.Norm() <= 1.0e-10){
                // APPROACH 1: concatenated trajectories
                // -------------------------------------
                // PlanConcatenatedSections();

                // APPROACH 2: use P2P
                // -------------------
                PlannerMethod original_method = method_;
                SetMethod(P2P);
                Plan();
...
void MotionPlanner::PlanARENA(){
    ...
    if (problematic_corridors.size() > 0){
        ...
        while (made_modification){
            ...
            std::chrono::duration<double, std::milli> solve_time = t1 - t0;
            if (!silent_mode_){ std::cout << "Solver time this iteration: " << solve_time.count() << " ms" << std::endl; }
            ...
            std::chrono::duration<double, std::milli> modification_time = t3 - t2;
            if (!silent_mode_){ std::cout << "Other time this iteration: " << modification_time.count() << " ms" << std::endl;}
        }
    }
...
void MotionPlanner::PlanConcatenatedSections(bool recursive){
    ...
    std::shared_ptr<const Trajectory> second_part_of_traj = last_solution_;
    ...
    if (!silent_mode_){std::cout << "Concatenation took " << duration.count() << " ms." << std::endl;}
