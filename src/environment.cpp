...
bool Environment::HasDeletedNeighbor(Point2D<int> cell) const {
    if (!isValidCell(cell)){
        throw InvalidEnvironmentOperationException("Cannot check neighbors of a cell outside of the environment");
    }
    ...
    for (int dx = -1; dx <= 1; dx++){
        for (int dy = -1; dy <= 1; dy++){
            ...
            Point2D<int> neighbor(cell.x() + dx, cell.y() + dy);
            if (isValidCell(neighbor) && 
                    occupancy_grid_[neighbor.x()][neighbor.y()] == DELETED){
                return true;
            }
        }
    }
...
bool Environment::ClaimDestination(
    ...
    // std::cout << "Destination " << name << " claimed by " << caller 
            //   << ": " << (successfully_claimed ? "SUCCESS" : "FAILURE") << std::endl;
    if (!successfully_claimed){
        // std::cout << "(destination is currently claimed by " 
        //           << claimable_destinations_[name].ClaimedBy() << ")" << std::endl;
    }
...
std::string Environment::GetNearestFreeClaimableDestination(const Point2D<double>& pos,
                                                            bool ignore_neighbouring_destinations) const {
    ...
    // check if the provided position is a claimable destination itself
    // if so, do not return a neighbouring destination (unless explicitly allowed)
    // Why do we do this: MultiMoverSimulator calls this function to figure out
    //     where to send agents to if deadlocks need to be resolved. If agents
    //     move up only one destination, there is no time to claim the newly
    //     released destination before the resolving agent claims it again.
    //     This results in oscillations. By not providing the resolving agent 
    //     a neighbouring destination, this situation is avoided.
    bool pos_is_destination = false;
    Point2D<int> cell = pos.ConvertWorldToCell(cell_width_, cell_height_);
    for (const auto &pair : claimable_destinations_){
        const ClaimableDestination &dest = pair.second;
        if (dest.DestinationIsAt(cell.x(), cell.y())){
            pos_is_destination = true;
            break;
        }
    }

    // std::cout << "Searching for nearest free claimable destination from position " 
    //           << pos << std::endl;
    for (const auto &pair : claimable_destinations_){
        const ClaimableDestination &dest = pair.second;
        if (!dest.Claimed()){
            ...
            if (distance < min_distance && 
                    !(ignore_neighbouring_destinations && pos_is_destination && 
                      distance <= 1.1*std::max(cell_width_, cell_height_))){
                min_distance = distance;
                nearest_destination = pair.first;
            }
        } else {
            // std::cout << "\tdestination " << pair.first << " is claimed by "
            //           << dest.ClaimedBy() << std::endl;
        }
    }
...
int Environment::GetNbDestinationsClaimedBy(const void* caller) const {
    int count = 0;
    for (const auto &pair : claimable_destinations_){
        if (pair.second.ClaimedByCaller(caller)){
            count++;
        }
    }
    return count;
...
void Environment::PrintDestinationsClaimedBy(const void* caller) const {
    std::cout << "Destinations claimed by " << caller << ":" << std::endl;
    for (const auto &pair : claimable_destinations_){
        if (pair.second.ClaimedByCaller(caller)){
            std::cout << "\t" << pair.first << " at " 
...
Point2D<int> Environment::GetRandomFreeCellPositionAtEnvironmentEdge() const {
    ...
    while (!isValidCell(cell) ||          // cell must be valid
            !IsFree(cell) ||              // cell must be free
           (cell.x() != 0 && cell.x() != nb_cell_cols_ - 1 && 
            cell.y() != 0 && cell.y() != nb_cell_rows_ - 1 && // cells at env edges are fine
            !HasDeletedNeighbor(cell))){  // cells next to deleted cells are fine
        cell.SetX(nb_cell_cols_*dis_x_(gen_));
        cell.SetY(nb_cell_rows_*dis_y_(gen_));
    }
...
void Environment::UpdateLocalView(Environment& env_view, const void* caller){
    env_view.ClearClaimableDestinations();
    // Claim destinations claimed by other objects
    for (const auto &pair : claimable_destinations_){
        const ClaimableDestination &dest = pair.second;
        // if (dest.Claimed()){
            if (!dest.ClaimedByCaller(caller)){
                env_view.AddObstacle(dest.GetLocation());
            } else {
                env_view.RemoveObstacle(dest.GetLocation());
            }
        // }
    }
}